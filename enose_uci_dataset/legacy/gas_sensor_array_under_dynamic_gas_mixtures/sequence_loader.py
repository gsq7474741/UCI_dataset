...
FLOAT_COLS = SENSOR_COLS
TS_COL = "t_s"
LABEL_COLS = ["ethylene_conc", "co_conc", "methane_conc"]
...
class SequenceCSVDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            usecols = self.float_cols + [self.ts_col] + self.label_cols
            df = pd.read_csv(path, usecols=usecols)
        except Exception:
            df = pd.read_csv(path)
        x = torch.tensor(df[self.float_cols].values, dtype=self.dtype)  # [T, C]
        # Sanitize any non-finite values early
        x = torch.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)
        y = torch.tensor([float(df[c].iloc[0]) for c in self.label_cols], dtype=torch.float32)  # [3]
        # Sanitize target to avoid NaNs/Infs in losses
        ...
        if self.ts_col and self.ts_col in df.columns:
            try:
                ...
                ts = None
        meta = {"id": os.path.splitext(filename)[0], "path": path, "length": x.shape[0]}
        if ts is not None:
            meta[self.ts_col] = ts
        return x, y, meta
    ...
    *,
    ...
    for fname in files:
        ...
        if baseline_subtract:
            ...
            x = x - baseline
        # One more sanitize before stats
        x = torch.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)
...
def _build_pad_collate(
    ...
    anchor_zero: bool = True, pre_s: float = 60.0, post_s: float = 240.0,
    ts_key: str = TS_COL, baseline_subtract: bool = True, baseline_s: float = 30.0,
):
    cols = list(float_cols or FLOAT_COLS)
    c = len(cols)
